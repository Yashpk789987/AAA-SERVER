...
router.post('/add_category' , (req , res) => {
  pool_1.query(`insert into category(english_name, hindi_name) values('${req.body.english_name}', '${req.body.hindi_name}')` , (err , result) => {
    if(err) throw err
...
router.post('/add_category/pg' , (req , res) => {
  pool_2.query(`insert into category(english_name, hindi_name) values('${req.body.english_name}', '${req.body.hindi_name}')` , (err , result) => {
    if(err) throw err
    res.json( { message : "Successfully Added ...."})
  })
})
...
router.post('/add_sub_category', upload.single('sub_category_image') , (req , res) => {
...
router.post('/add_sub_category/pg' ,  upload.single('sub_category_image') , (req , res) => {
  let data = JSON.parse(req.body.SendData)
  let query = `insert into sub_category(english_name, hindi_name, category_id , logo) values('${data.english_name}', '${data.hindi_name}'
  , ${data.category_id} , '${req.file.filename}')`
...
router.get('/fetch_all_sub_categories/pg' , (req , res) => {
  pool_2.query(`select * from sub_category` , (err , result) => {
    if(err) throw err
    res.json(result.rows)
  })
})
...
router.get('/sub_categories/:category_id' , ( req , res) => {
  pool_1.query(`select * from sub_category where category_id = ${req.params.category_id}` , (err , result) => {
      if(err) throw err
      res.json(result)
  })
})
